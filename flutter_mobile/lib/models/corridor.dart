...
class AnalyticsSummary {
  final int totalVehiclesEvaluated;
  final int activeEmergencyVehicles;
  final int totalAlertsDispatched;
  final double averageClearanceTimeSeconds;
  final Map<String, dynamic>? dbStatus;

  AnalyticsSummary({
    required this.totalVehiclesEvaluated,
    required this.activeEmergencyVehicles,
    required this.totalAlertsDispatched,
    required this.averageClearanceTimeSeconds,
    this.dbStatus,
  });

  factory AnalyticsSummary.fromJson(Map<String, dynamic> json) {
    return AnalyticsSummary(
      totalVehiclesEvaluated: (json['totalVehiclesEvaluated'] as num?)?.toInt() ?? 0,
      activeEmergencyVehicles: (json['activeEmergencyVehicles'] as num?)?.toInt() ?? 0,
      totalAlertsDispatched: (json['totalAlertsDispatched'] as num?)?.toInt() ?? 0,
      averageClearanceTimeSeconds: (json['averageClearanceTimeSeconds'] as num?)?.toDouble() ?? 0.0,
      dbStatus: json['dbStatus'] as Map<String, dynamic>?,
    );
  }
}
