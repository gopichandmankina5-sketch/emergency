class CorridorAlert {
  ...
  factory CorridorAlert.fromJson(Map<String, dynamic> json) {
    return CorridorAlert(
      ...
      action: json['action']?.toString() ?? json['assignedAction']?.toString() ?? 'NO_ALERT',
      actionText: json['actionText']?.toString() ?? _defaultActionText(json['action']?.toString() ?? json['assignedAction']?.toString() ?? 'NO_ALERT'),
